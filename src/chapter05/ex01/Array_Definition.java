...
public class Array_Definition {
	...
	public static void main(String[] args) {
		/*
		 	자료형: 자바는 변수명 앞에 반드시 자료형이 선언되어야 한다.
		 		- 기본 자료형(8가지): boolean, 정수형(byte, short, int, long), 실수형(float, double), 문자(char)
		 			- 변수명과 값이 Stack 메모리 영역에 저장됨
		 			- 사용 전에 반드시 초기값이 할당 되어 있어야 한다.
		 			- 첫자가 소문자로 되어 있다.
		 		...
		 		- 참조 자료형: 배열 / 열거 / 객체(class) / 인터페이스
		 			...
		 			- 값 (Heap)
		 			- 참조 자료형에서 ==		<== Stack의 값을 비교하는 것(Heap의 주소를 비교)
		 				Heap의 값을 비교할 때는 equals()를 사용해야한다.
		...
		// 3. 배열의 기본 값을 출력: 선언만 해도 강제로 초기값이 Heap에 들어감
		System.out.println(arr1[0]);		// arr1 배열 변수의 0번방의 값을 출력
		System.out.println(arr1[1]);		// arr1 배열 변수의 1번방의 값을 출력
		System.out.println(arr1[2]);		// arr1 배열 변수의 2번방의 값을 출력
	//	System.out.println(arr1[3]);		// arr1 배열 변수의 3번방의 값을 출력
		...
		arr2[4] = 50.0;
		arr2[5] = 60.0;
	//	arr2[6] = 60.0;			// ArrayIndexOutOfBound Exception (배열의 방번호를 알 수 없다.)
	...
	//	System.out.println("arr2의 5번째 방의 값" + arr2[6]);
		...
		for (int i=0; i<100; i++) {
			arr3[i] = i+1;					// 배열의 각 방에 1[0], 2[1], 3[2], 4[3], 5[4], ... 100[99]
		}
