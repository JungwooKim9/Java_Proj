...
public class Rectangle_Array_Output {
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<arr1.length; i++) {			// 행을 Loop:			0, 1, 2
			...
			if (i == 0) {			// 1, 2, 3, ... 100
				for (int j=0, a=1; j<arr1[i].length; j++, a++) {	// 행에 맞춰 열을 Loop:	0, 1, 2, ... 99
					// 값을 넣을 변수
					arr1[i][j] = a;
					//arr1[i][j] = j+1; <= 대체 가능
				}
		...
		System.out.println("===1. 이중 For문을 사용해서 2차원 정방형 배열 출력===");
		...
		System.out.println("===2. Enhanced For문을 사용해서 2차원 정방형 배열 출력===");
		for (int[] k : arr1) {
			for (int a : k) {
				System.out.print(a + " ");
			}
		...
		System.out.println("===1. Arrays.toString()을 사용해서 2차원 정방형 배열 출력===");
		// Arrays.toString()
