...
class B {
	...
	int m;				// 인스턴스 필드: 객체화 후 사용 가능, 다른 객체와 별도로 분리가 됨, Heap
	static int n;		// 정적(static) 필드: 객체 없이 클래스명으로 사용 가능, 객체화해서도 사용됨, 메모리에 클래스 영역에 저장됨
								// 모든 객체와 공유되는 빌드
...
public class Field02 {
	...
	public static void main(String[] args) {
		...
		// 인스턴스 필드: m		<== 객체 내부에서 고유 공간, 다른 객체화 같이 공유되지 않는 필드
		b1.m = 1;
		b2.m = 2;
		b3.m = 3;
		...
		B.n = 30;		// 객체 생성 없이 클래스 이름으로 사용 가능
