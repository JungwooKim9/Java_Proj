...
public class Method04 {
	...
	String sum (int a, int b) {
		...
		int c = a + b;			// c = a + b
		String d = String.valueOf(c);	// 정수 c를 String 타입으로 변환 후 d 변수에 할당
		return d;
	}
	...
	public static void main(String[] args) {
		...
		// sum2: static (정적 메소드)
		int m = Method04.sum2(10, 20);		// 1. 클래스명으로 호출: 객체 생성 없이 호출 됨
		...
		int k = sum2(10,20);				// 3. main메소드에서 메소드명으로 바로 호출 됨:
		System.out.println(k);						// 조건: 동일한 클래스 내부, static
		
	//	sum(10,10);			인스턴스 메소드: 객체 생성 후 호출 가능
		sum2(10,10);		// 정적 메소드: 객체 생성 후 호출, 클래스명으로 호출, 메소드명으로 바로 호출(같은 클래스에 있을시)
	}
