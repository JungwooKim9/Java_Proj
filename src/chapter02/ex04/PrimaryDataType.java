...
public class PrimaryDataType {
		...
		/*
			자료형 :
				- 기본 자료형: RAM STACK (스택) 공간에 변수명, 변수의 값이 저장
					- 8가지 기본 자료형: boolean, 정수(bite, short, int, long), 실수(float, double), 문자(char)
					- 정수형 자료형의 기본은: int			<== long 선언 후 변수의 값(리터럴)을 담을 때, 뒤에 l or L
					- 실수형 자료형의 기본은: double		<== float 선언된 변수에 값을 넣을 때 뒤에 f or F
				- 참조 자료형: RAM STACK (스택) 공간에 힙에 저장된 참조 주소를 가지고 있다. Heap (힙)에 값이 저장
					- 무한대 (객체 (String) / 배열 / ...)
					
			// 리터럴: 변수에 할당된 값을 통칭해서 리터럴이라 함
					
			*/
		
		
		// 1. boolean: true or false 값만 저장할 수 있는 자료형
	...
	// 	boolean bool3 = 123;		// 오류 발생
		...
		// 상수: 변수 이름 앞에 final 키가 붙은 것, 상수명은 변수명과 구분하기 위해서 대문자로 사용
	...
	//	BOOL4 = true;				// 오류 발생: 상수는 값을 수정할 수 없다.
		...
		// long: 8byte ( = 64bit, -2^63 ~ +2^63-1 )
					...
					// long 형으로 선언된 변수에 값을 할당할 때 주의, 리터럴의 값을 넣을 때: L, l 붙여줘야함
		
		long val7 = -9223372036854775808L;
		long val8 = 9223372036854775807L;
		
		long val9 = 123456;		// int 자료형이 담을 수 있는 값보다 큰 값을 넣을 때 L

		// 3. 실수형 자료형		:double이 기본이어서, float 자료형에 값을 할당할 때 f, F를 붙여줘야함
		//		-- float: 4byte,	소수점 7자리까지 정밀도를 유지
		//		-- double: 8byte,	소수점 15자리까지 정밀도를 유지
		...
		// 4. 문자 (char): 1글자,		문자열: 1글자 이상, String(참조 자료형, 객체), 리터럴에 값을 넣을 때 " "
			// -- 리터럴에 값을 넣을 때: ' '
			// -- 리터럴에 문자: A, B, C , a, b, c
			// -- 리터럴에 특수 문자: #, !, %
			...
			// -- 한글 1자, 중국어 1자, 일본어 1자
		// 직접 문자를 대입: 영어(대문자, 소문자), 숫자, 특수문자		<== 7byte 코드값: 아스키 코드
		...
		// 1. println(): 문자열과 변수를 출력시 + 연결 사용
			System.out.println("나의 이름은 " + n1 + " " + n2 + n3 + "입니다.");
		
		// 2. print(): 문자열과 변수를 출력시 + 연결 사용, \n으로 개행
			System.out.print("나의 이름은 " + n1 + " " + n2 + n3 + "입니다.\n");
		
		// 3. printf(): 문자열 내에서 , 뒤에 변수 값을 불러들임 "%s": 문자열,	"%d": 정수,	"%5.2f": 실수
				// "%c": 문자 변수 값을 불러들임
