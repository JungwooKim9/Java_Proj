...
// 부모 클래스 (Human)		<== 필드와 메소드: 자식 클래스에서 공통된 특징을 갖는 필드와 메소드를 정의
class Human {
...
public class Inheritance01 {
	...
	public static void main(String[] args) {
		/*
		 상속 (Inheritance): 부모 클래스(객체화)와 필드(속성)와 메소드(기능), 이너클래스를 자식 클래스에서 상속 받아서 사용.
		 	- 생성자는 상속되지 않는다.
		 	- 중복된 코드를 방지 할 수 있다. 코드가 간결해지고 생산성이 향상됨
		 	- 객체 지향 언어의 꽃이다.
		 	- 다형성: 부모 클래스의 내용을 상속 받아서 다양한 형태로 출력: 동물(부모 클래스) <== 사람, 독수리, 사자, 호랑이, 개, 고양이
		 			-- 부모 클래스의 필드와 메소드는 자식 클래스로 내려감.
		 	- 배열이나 컬렉션에 담아서 사용(부모 클래스의 타입으로 업캐스팅 해서 저장)
		 		-- 배열이나 컬렉션에 담긴 내용을 다운 캐스팅해서 사용.
		 	- 자바는 다중 상속이 지원되지 않음. (자식클래스는 하나의 부모 클래스만 가진다.)
		 */
		...
		// 메소드
		s1.eat();				// Human 클래스의 필드
		s1.sleep();				// Human 클래스의 필드
		s1.goToSchool();		// Student 클래스의 필드
