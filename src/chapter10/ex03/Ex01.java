...
// 보모 클래스: 자식 클래스의 공통적으로 적용되는 필드, 메소드 정의
class Human {		// (부모 클래스)사람 <==> (자식 클래스) 학생, 교수, 근로자
	
	// 기본 생성자가 생략되어 있다.
	// Human () {}
	
	// 필드: 상속 받을 자식 클래스의 공통으로 사용하는 속성을 정의
	String name;
	int age;
	
	// 메소드
	void eat () {
		System.out.println("Human - 모든 사람은 먹습니다.");
	}
}

// 자식 클래스
class Student extends Human {
	// 기본 생성자가 생략되어 있다.	<= 상속 관계에서는 기본 생성자 내부에 부모 클래스에 기본 생성자를 호출
	Student () {
...
class Professor extends Human {
	...
	void goToTeach () {
	System.out.println("Professor - 교수는 학생을 가르치러 학교에 갑니다");
}
	
	// 메소드 오버라이딩: Human의 eat() 메소드를 재정의
	@Override
	void eat() {
...
class Worker extends Human {
	...
	int workerID;
	void goToWork () {
		System.out.println("Worker - 근로자는 일터로 출근합니다.");	
	}
	
	// 메소드 오버라이딩
	@Override
	void eat() {
		System.out.println("Worker - 근로자는 식사를 합니다");
	}
...
public class Ex01 {
	...
	public static void main(String[] args) {
		...
		// h1은 Human을 객체화 했게 때문에 Student / Professor / Worker 클래스를 내포하고 있지 않아서 다운캐스팅 할 수 없다
		
		// 다운캐스팅 오류 발생: 명시해야 한다.	<== 주의: 런타임 오류 발생: instanceof를 사용해서 해당 타입이 존재하는지 확인 후 캐스팅 필요
		...
		Student ss1 = (Student) h1;		// h1에 Student 타입이 존재할때만 실행
			...
			System.out.println("h1에는 Student타입이 존재하지 않습니다.");
		}
		...
		// ss2: Human / Student 내포하고 있고 Student 타입으로 다운 캐스팅: Human / Student 필드 메소드를 모두 사용 가능
		ss2.name = "세종대왕";		// Human
		ss2.age = 50;			// Human
		...
		Human hh2 = ss2;		// 업캐스팅은 캐스팅시 instanceof를 사용하지 않아도 됨. 컴파일 단계에서 오류 발생
		
		// 자식을 객체화해서 부모 타입으로 지정 후 배열 / 컬렉션에 저장 후에 끄집어내서 출력함
		
		Student s10 = new Student();		// Human / Student
		Professor p10 = new Professor();		// Human / Professor
		Worker w10 = new Worker();		// Human / Worker
		...
		// s10, p10, w10 자동으로 upcasting되어서 Human타입으로 저장됨
		...
		// 배열의 각 방의 내용을 끄집어 낼 때 다운 캐스팅하면서 끄집어 낸다.
		Student s20 = (Student) arr[0];		// Human ==> Student로 다운 캐스팅
		Professor s21 = (Professor) arr[1];	// Human ==> Professor로 다운 캐스팅
		Worker s22 = (Worker) arr[2];		// Human ==> Worker로 다운 캐스팅
		...
		Student s30 = new Student();	// s30: Human / Student
		...
		w30.eat();		// Worker 클래스의 eat(); 메소드 호출
		...
		h40.eat();		// Human 클래스의 eat() 메소드 호출시, Student 클래스의 오버라이딩 된 eat()이 작동됨
		h41.eat();		// Human 클래스의 eat() 메소드 호출시, Professor 클래스의 오버라이딩 된 eat()이 작동됨
		h42.eat();		// Human 클래스의 eat() 메소드 호출시, Worker 클래스의 오버라이딩 된 eat()이 작동됨
		
		// 부모의 메소드 eat()을 호출시 자식 클래스의 오버라이딩 된 eat() 메소드가 출력됨	<== 매우 매우 중요함
