...
public class Method_Overriding01 {
	...
	public static void main(String[] args) {
		/*
		 메소드 오버로딩(Method Overloding) - 상속과는 상관 없음
		 	- 메소드 이름은 동일. 시그니쳐에 따라서 다른 메소드를 식별 작동
		 	- 시그니쳐: 메소드를 식별하는 식별자, 리턴타입(x), 아규먼트의 갯수, 아규먼트의 자료형
		 메소드 오버라이딩(Method Overriding) - 반드시 상속 관계에서만 설정할 수 있음
		 	- 부모 메소드의 실행부를 재정의 해서 사용하는 것
		 	- 인스턴스 메소드일때만 오버라이딩이 됨
		 	- 필드는 오버라이딩 되지 않는다.
		 	- 스태틱 메소드도 오버라이딩 되지 않는다.
		 	- @Override 어노테이션: 부모 클래스의 메소드를 오버라이딩 하겠다고 선언을 하는 선언자
		 			-- 부모 클래스의 메소드의 리턴 타입, 메소드명, 시그니쳐(인풋갯수,자료형)가 정확해야한다.
		 */

		// A 클래스를 객체화 해서 A타입으로 지정함
		A a = new A();		// a: A타입만 존재함
		a.print();
		System.out.println("====================");
