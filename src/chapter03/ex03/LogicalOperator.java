	...
	public static void main(String[] args) {
		...
		/*
		 논리 연산자: 논리 연산한 결과를 true, false		<== if, 에서 조건으로 처리됨
		 	&&(AND): 
		 	||(OR): 
		 	^ (XOR): 앞 뒤가 같을 때 false를 리턴, 앞 뒤가 다를 때 true를 리턴
		 	! (NOT): 
		 */
		...
		// ^: XOR: 앞 뒤의 값이 같을 때 false(0), 앞 뒤의 값이 다를 때 true(1)
		...
		// 숏트 서킷이 발생될 수 있다.
			// -- 논리 연산에서 앞의 값으로 결과 값을 유추할 수 있는 경우 뒤의 내용을 무시해서 처리하지 않는다.
		
		// && 연산에서 false가 오면 뒤에는 어떤 값이 와도 false이기 때문에 쇼트 서킷이 발동
		System.out.println(true && true);	// 쇼트 서킷이 발생되지 않는다.
		System.out.println(false && true);	// 쇼트 서킷이 발생 된다. 앞의 값이 false이면 뒤에 값은 확인하지 않아도 false
		System.out.println("============================================");
		
		// 예제: AND 연산에서 쇼트 서킷이 발생되는 예제
		int val1 = 3;
		System.out.println(false && ++val1 > 6);	// false 뒤에 값은 확인 안 함
