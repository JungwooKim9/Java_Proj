...
class GenericMethod {	// 일반 클래스
	...
	// 제너릭 메소드는 반드시 일반 클래스에 생성 가능하다.(제너릭 클래스에 불가)
	...
	public <K, V> void method4 () {
		System.out.println("반드시 타입을 지정해서 호출 되어야함: 매개변수에서 타입을 호출 할 수 없기 때문");
	}
...
public class GenericMethod01 {
	...
	public static void main(String[] args) {
		/*
		 제너릭 메소드:
		 	- 주의: 제너릭 클래스 내부의 메소드가 아니다.	제너릭 클래스 내부에는 제너릭 메소드를 생성할 수 없다.
		 	- 일반 클래스의 제너릭 메소드가 생성 된다.
		 	- 1. 객체를 생성해서 메소드를 호출시 <타입> 넣고 호출
		 	- 2. 제너릭 메소드의 인풋값이 타입을 유추할 수 있는 경우: 메소드 호출시 타입 생략 가능 하다.
		 */
		
		// 제너릭 메소드 호출:
		GenericMethod gm = new GenericMethod();		// 일반 클래스를 객체화
		String str1 = gm.<String> method("안녕");		// 제너릭 메소드: 메소드를 호출시 타입을 지정해서 호출
		String str2 = gm.method("오늘");		// 매개변수로 들어오는 값이 타입을 유추 할 수 있을 때 메소드 호출시 타입을 생략할 수 있다.
