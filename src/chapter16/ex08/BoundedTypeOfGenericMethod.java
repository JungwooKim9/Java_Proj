...
class A {	// 일반 클래스의 제너릭 메소드
	...
	public <T extends Number> void method1(T t) {	// 정수나 실수를 처리하는 자료형만 올 수 있다.
		System.out.println(t.intValue());	// t.intValue(): Number 타입의 메소드
	}	
}

// 인터페이스: 제너릭 타입 제한시 인터페이스일 때 다중 제한이 가능하다.		클래스일 때는 하나만 처리할 수 있다.
		// < T extends IF1 & IF2 & IF3 >
		// 인터페이스로 제한을 두면 인터페이스를 구현한 하위의 모든 클래스가 타입으로 지정될 수 있다.
interface IF1 {
	void print();		// public abstract가 생략됨. 구현 방법 1. 자식 클래스에서 구현 클래스를 만들고 사용 2. 익명 클래스를 사용해서 구현
}
...
class Aaa extends Aa {
	@Override
	public void print() {
		System.out.println("Aa의 자식 Aaa의 print2 메소드 - 오버라이딩");
	}
	...
	public static void main(String[] args) {
		/*
		 제너릭 메소드에서 타입 제한
		 	...
		 	- 타입 제한을 두면: 제한을 둔 클래스의 메소드를 사용 가능
		 */
