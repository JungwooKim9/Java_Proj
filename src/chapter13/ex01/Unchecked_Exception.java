...
public class Unchecked_Exception {
	...
	//	System.out.println(3 / 0);
		...
		// 2. ClassCastException: 다운 캐스팅시 객체 내부에 해당 타입이 존재하지 않을 떄 발생되는 예외
		A a1 = new B();		// a1: A / B
		A a2 = new C();		// a2: A / C
		
	//	C c1 = (C) a1;
		
		// 3. ArrayIndexOutOfBoundException: 배열의 방 번호를 넘어간 경우 발생되는 예외
					// index   0 1 2 3 4
		int[] arr = new int[] {1,2,3,4,5};
		
	//	System.out.println(arr[5]);		
		
		// 4. NumberFormatException: 문자 타입의 정수나 실수를 정수나 실수로 바꿀 때 발생되는 오류
		String a = "10!";
	//	int num = Integer.parseInt(a);		// 문자 => 정수, 실수
		
		// 5. NullPointerException: 참조 주소의 값이 Null인데, 메소드 호출하는 경우
		String str = null;
