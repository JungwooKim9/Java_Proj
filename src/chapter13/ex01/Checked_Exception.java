...
public class Checked_Exception {
	...
	public static void main(String[] args) {
		/*
		 1. Error vs Exception
		 	...
		 	- Exception: JVM 위에서 자바 프로그램에서 발생되는 문제들을 예외(Exception)
		 		- 개발자가 제어가 가능한 부분
		 		- Exception이 발생되면 그 시점에서 프로그램이 중지되어 버림
		 		- Exception이 발생 되더라도 프로그램이 종료되지 않도록 처리해야함
		...
		// 1. InterrupedException: 인터럽트가 발생 되었을 때 예외
	//	Thread.sleep(1000);			// 스레드를 1초 동안 잠시 멈추어라
		
		// 2. ClassNotFoundException: 특정 경로에 해당 클래스가 존재하지 않는 경우 발생되는 예외
	//	Class cls = Class.forName("java.lang.Object");		// DB의 driver 클래스를 로드할 때 (Oracle / MS-SQL / MySQL)
	...
	//	in.read();		// IO에 대한 오류
		...
		// 4. FileNotFoundException: 파일을 찾을 수 없을 때 발생하는 오류
