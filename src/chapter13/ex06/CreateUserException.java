...
class A {
	...
	// 정수를 인풋 받아서 70이하이면 예외를 강제로 발생 시킴(예외를 직접 처리)
	void abc_1(int num) {
		...
		if(num > 70) {
			// 정상 처리
			System.out.println("당신이 넣은 값은: " + num + "이고 정상 작동됨");
		}else {
			// 예외를 강제로 발생 시킴
			throw mre2;			// RuntimeException
				...
				System.out.println(e.getMessage());			// 실행 예외가 발생 되었을 때 예외 메세지를 출력 하라
			}
	}
	...
	// 정수를 인풋 받아서 70이하이면 예외를 강제로 발생 시킴(예외를 미루기)
	void abc_2(int num) throws MyException {	// abc_2() 메소드를 호출하는 곳에서 예외를 처리해야함.
		if (num > 70) {
			...
			// 예외를 강제 발생 (throws: 예외를 미루기, throw: 예외를 강제로 발생 시킴)
			throw mre1;		// 일반 예외
	...
	void bcd_2() {
		...
		try {
			...
			System.out.println(e.getMessage());		// MyException의 생성자에 넣은 오류 메세지를 출력 해라
		}
	...
	public static void main(String[] args) {
		/*
		 사용자 정의 예외: Java에서 제공되지 않는 예외를 자신이 직접 만들어서 사용하는 예외
		 	- 일반 예외 (Checked Exception): 컴파일 단계에서 오류가 발생한다. 예외 처리가 반드시 필요
		 		- Exception 클래스를 상속 해서 사용자 정의 예외를 생성 (1. 기본 생성자 2. 오류 메세지를 처리할 생성자)
		 	- 실행 예외 (Unchecked Exception, Runtime Exception): 실행시 예외가 발생한다. 
