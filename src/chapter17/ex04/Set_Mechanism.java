...
class A {		// data 필드에 들어오는 값으로 각 객체를 식별하기 위해서는: hashCode(), equals()가 재정의 되어있어야 set에 넣을 때
...
// hashCode(), equals() 메소드 재정의: data 필드의 값이 동일할 때 hashCode가 동일
class B {
	int data;
	...
	public boolean equals(Object obj) {		// b1.equals(b2)
		if (obj instanceof B) {		// 다운캐스팅 하기 전에 해당 타입이 존재하는지 확인 처리
	...
	// 2. hashCode()를 재정의: data 필드의 값으로 hashCode를 생성하도록 재정의
		// data 필드의 값이 동일하면 동일한 hashCode가 생성됨
	@Override
	public int hashCode() {
	...
	public static void main(String[] args) {
		/*
		 Set<E>: 1. 방번호[index]가 존재하지 않는다. 2. 중복된 값을 넣을 수 없다.
		 	- Set에 중복된 값을 넣지 못하도록 하기 위해서는 Object에 두 개의 메소드가 재정의 되어 있어야 한다.
		 		- 객체의 특정 필드의 hashCode(), equals()가 재정의 되어있어야 한다.
		 		- Wrapper Class: Boolean, Byte, Short, Integer, Long, Float, Double, Charter, String
		 		- 자신이 생성한 특정한 클래스(Type)를 set에 넣을 경우: Object의 hashCode(), equals() 메소드를 재정의 해야 한다.
		 */
		...
		// 3. Set의 B: data 필드에 대해서 hashCode(), equals() 재정의 함.
