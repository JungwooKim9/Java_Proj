	...
	public static void main(String[] args) {
		/*
		 List<E>: 인터페이스, 메소드만 선언되어 있다. 구현되어 있지 않다.
		 	...
		 	: 컬렉션 값을 추가하거나, 제거하거나, 수정할 때 메소드를 사용함
		 	
		 	-- ArrayList<E>		: 싱글 쓰레드
		 	-- Vector<E>		: 멀티 쓰레드 환경, 모든 메소드가 동기화 처리가 되어 있기 때문에 멀티 쓰레드 환경에 유리
		 		-- 중간에 어떤 값을 넣거나 제거할 경우, 부하가 많이 발생 된다.
		 		-- 빈번하게 값을 수정하는 경우 부하가 많이 발생 된다.
		 	-- LinkedList<E>
		 		-- 중간에 어떤 값을 추가/제거 하더라도 부하가 많이 발생되지 않는다.
		 		-- 빈번하게 값이 변경되는 경우 사용하는 자료형
		 */

		// List에서 사용 되는 메소드
		
		// 자식의 구현 클래스를 객체화 해서 List 타입으로 선언: 인터페이스의 메소드를 호출하면 오버라이딩된 메소드 호출
		...
		// 삭제
		// 6. remove(int index): index 방번호의 값을 삭제, 삭제 후 뒤의 값이 한 칸씩 앞으로 당겨지게 된다.
		aList3.remove(2);
		...
		// 13. toArray(T[] t): 리스트 ===> 배열, T[]	<== 리스트를 배열로 끄집어낼 때 해당 타입의 배열로 바로 끄집어 낸다.
		
		Integer[] int1 = aList2.toArray(new Integer[0]);	// int[] int1 = new int[방크기]
