...
interface A {
	...
	// default 메소드: 인터페이스 내부에서 구현부를 가지는 메소드
	default void cde() {		// 2010년도에 구현된 메소드: USB 3.0		<== 새로운 기능을 1000개의 클래스에 적용하려고 한다.
		System.out.println("2010년도에 USB 3.0 기능 구현");
	}
	
	// static 메소드: 인터페이스 내부에서 구현부를 가지는 메소드
	static void def () {
		System.out.println("static 메소드 - 객체 생성 없이 타입으로 바로 호출 가능");
	}
	
}
// 위 인터페이스를 구현한 클래스가 1000개 있다고 가정
...
public class Default_Method {
	...
	public static void main(String[] args) {
		/*
		 인터페이스의 특징: 
		 	...
		 	- 모든 필드는 public static final이 생략되어 있음
		 	- 모든 메소드는 public abstract가 생략되어 있음
		 	- 인터페이스 내부의 메소드는 선언만 되어야하고 구현부{}를 생성하면 안된다.	<== 자바 1.7까지
		 		- default / static	<== 구현부를 가지는 메소드		<== 자바 1.8부터 추가된 기능
		 default: 메소드 앞에 붙이는 키	<== 인터페이스에서 구현부가 있는 메소드, 주의: 접근 제어자가 아니다
		 	- 인터페이스에서 구현된 모든 클래스에게 상속되어서 적용
		 	- 새로운 기능을 하위의 모든 클래스에게 적용할 때 사용함. 오류 없이 적용할 때 사용
		 	- 오버라이딩도 가능함
		 static 메소드: 인터페이스 내에서 메소드 이름 앞에 붙임. 구현부가 있는 메소드
		 	- 자식 클래스의 객체를 생성하지 않고 인터페이스 이름으로 호출 가능한 메소드
		 */
		
		// 자식 클래스 객체를 생성하면서 인터페이스 타입으로 지정
		A a1 = new Aa();
		A a2 = new Aaa();
	...
	//	A.a = 50;		// 인터페이스의 필드는 final이므로 값 수정 불가
