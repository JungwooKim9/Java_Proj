...
// 2. 익명 클래스로 사용하는 방법: 한 번 사용하고 더 이상 사용하지 않을 경우
abstract class C {
	abstract void cry();
}
		...
		/*
		 추상 클래스 (Abstract Class)
		 	...
		 	- 추상 클래스를 작동하는 방법은 2가지
		 		...
		 		- 2. 콘크리트 클래스 없이 익명 클래스를 만들어서 사용	<== 한 번 사용하고 버리는 경우,
		 				안드로이드 폰 개발시 이벤트 처리할 때 많이 사용
		 */
		
		// 1. 구현 클래스를 만들고 사용하는 방법 (콘크리트 클래스를 객체화 해서 타입을 추상 클래스로 지정 후 메소드 호출)
		A a1 = new B();
		a1.cry();
